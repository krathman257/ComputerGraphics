...
class Renderer {
    ...
    glm::vec3 shade(const Camera& camera, const Lights& lights, const Hit& hit) {
        glm::vec3 color = camera.getBackground();
    ...
    glm::vec3 render_pixel(
        ...
        Hit hit = _intersector->find_first_intersection(bb, ray, 20);
...
public:
    ...
    void render(
        ...
        for (int y = 0; y < image.height(); ++y) {
            std::cout << 100 * (float)(y) / (float)(image.height()) << std::endl;
